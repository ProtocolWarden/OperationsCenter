...
| 2 | **`RuntimeBindingPolicy` (per-task model-tier selection) is dead** — coordinator defaults it to None; the advertised activation method from_defaults_with_runtime_policy() **does not exist** (the only reference is a comment). So `request.runtime_binding` stays None and tier is always the static team default. | `policy/runtime_binding_policy.py`; `execution/coordinator.py:101,116` | ~~DEAD~~ **WIRED (S3)** | RESOLVED: added `ExecutionCoordinator.from_defaults_with_runtime_policy()` + `resolve_runtime_binding_policy(settings)`; `execute/main.py` now passes the resolved policy in. FAIL-SAFE: `settings.runtime_binding.enabled=False` (default) → `None` → static-team behavior. When enabled, model tier varies per (task_type, lane) — opus for refactor/feature, sonnet for tests, haiku for lint. Policy from `runtime_binding.policy_path` (yaml, relative-to-config) or bundled `DEFAULT_POLICY`. |
| 3 | **Recovery loop is permanently single-attempt** — `RecoveryPolicy()` defaults `max_attempts=1`, no config wires it up, so retry/backoff/RATE_LIMIT-retry_after/budget-checker code is unreachable. (`policy.py:84` even shapes `_ALWAYS_REFUSE` to dodge a hollow-return static check — a "looks-implemented" tell.) | `execution/coordinator.py:121`; `execution/recovery_loop/*` | ~~DEAD~~ **WIRED (S3)** | RESOLVED: added `RecoverySettings`; `execute/main.py` builds `RecoveryPolicy` from `settings.recovery.*`. FAIL-SAFE: default `max_attempts=1` (single-shot, current behavior). SIDE-EFFECT SAFETY is already enforced by the engine (Rule 6 `STOP_IDEMPOTENCY_REQUIRED`): non-idempotent requests are NOT retried even with `max_attempts>1`; live requests default `idempotent=False`. When raised, transient failures retry with bounded backoff. |
| 4 | **`QueueHealingEngine` (5-rule blocked-queue healer) is never run** — its only consumer is the `operations-center-triage-scan` verb, which is NOT registered in `spec_hygiene` maintenance and invoked by no systemd unit / watchdog. | `queue_healing/engine.py:12`; `maintenance/triage_scan.py:303` | ~~DEAD~~ **WIRED (S3)** | RESOLVED: `QueueHealingTask` (`entrypoints/maintenance/queue_healing_task.py`) wraps the tested triage_scan helpers (`_queue_healing_actions` + extracted `apply_queue_healing_actions`) and is registered in `spec_hygiene` `register_maintenance_tasks`. NON-DESTRUCTIVE (only NONE/ESCALATE-comment/BLOCKED→READY/BLOCKED→BACKLOG; never deletes) + idempotent. FAIL-SAFE: registered DISABLED (`settings.queue_healing_enabled=False` default → registry never schedules it); no-op on a clean board even when enabled. |
| 5 | **`recovery/` (parked-state machine, store, 21-field telemetry) + `recovery_policies/` budget tracker — built, test-covered, never wired** — full reference set is the packages + a single isolation test. | `recovery/parked.py`, `recovery/telemetry.py`, `recovery_policies/budget.py` | ~~DEAD~~ **WIRED (S3)** | RESOLVED: `ParkedUnparkTask` (`entrypoints/maintenance/parked_unpark_task.py`) loads `ParkedStateStore`, recomputes the board evidence fingerprint, calls `should_unpark`, drives `RecoveryBudgetTracker`, and clears (unparks) / refreshes the sidecar; registered in `spec_hygiene`. FAIL-SAFE: EMPTY STORE → no-op (the healthy-fleet case; no producer parks anything yet); registered DISABLED (`settings.parked_unpark_enabled=False` default). Only ever clears/refreshes the parked sidecar — never touches Plane work. (`recovery/telemetry.py` `WatcherRecoveryTelemetry` remains a passive dataclass — display/record only, no new consumer.) |
