...
def check_missing_files(index: "ManagedArtifactIndex") -> list[CalibrationFinding]:
    ...
    absent = [
        a for a in index.artifacts if a.exists_on_disk is False and a.resolved_path is not None
    ]
    if not absent:
        return []
...
def check_singleton_limitations(index: "ManagedArtifactIndex") -> list[CalibrationFinding]:
    ...
    overwritten = [a for a in singletons if Limitation.REPO_SINGLETON_OVERWRITTEN in a.limitations]
    if overwritten:
        findings.append(
            CalibrationFinding(
                severity=FindingSeverity.INFO,
...
def check_producer_compliance(index: "ManagedArtifactIndex") -> list[CalibrationFinding]:
    ...
    unknown_content = [a for a in index.artifacts if a.content_type in ("unknown", "")]
    if unknown_content:
        findings.append(
            CalibrationFinding(
                severity=FindingSeverity.WARNING,
    ...
    no_description = [a for a in index.artifacts if not a.description]
    if no_description:
        findings.append(
            CalibrationFinding(
                severity=FindingSeverity.INFO,
    ...
    no_consumer = [a for a in index.artifacts if not a.consumer_types]
    if no_consumer:
        findings.append(
            CalibrationFinding(
                severity=FindingSeverity.INFO,
    ...
    no_valid_for = [a for a in index.artifacts if not a.valid_for]
    if no_valid_for:
        findings.append(
            CalibrationFinding(
                severity=FindingSeverity.INFO,
...
def check_content(
    ...
    readable = [a for a in candidates if a.resolved_path is not None and a.exists_on_disk is True]
