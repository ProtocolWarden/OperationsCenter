...
class TestSignalQuery:
    ...
    def get_flaky_tests(self, timerange: TimeRange | None = None) -> list[FlakyTest]:
        """Get all flaky tests detected in a time range.

        Extracts flaky test information from FlakyTestSignal in snapshots,
        converting them to structured FlakyTest results for autonomy systems.
        ...
        Example:
            flaky_tests = query.get_flaky_tests(TimeRange.last_days(7))
            for test in flaky_tests[:10]:
                print(f"{test.name}: {test.failure_rate*100:.1f}% failure rate")
        """
        if timerange:
            snapshots = self._load_snapshots_in_range(timerange)
        else:
    ...
    def get_test_metrics(self, timerange: TimeRange | None = None) -> FlakyTestMetrics | None:
        """Get aggregated flaky test metrics for a repository.

        Synthesizes FlakyTestSignal data across a time range to compute
        repository-level flakiness metrics and trends.
        ...
        Example:
            metrics = query.get_test_metrics(TimeRange.last_days(30))
            if metrics:
                print(f"Total flaky tests: {metrics.total_flaky_tests}")
                print(f"Trend: {metrics.trend:+.1f}% change")
        """
        if timerange:
            snapshots = self._load_snapshots_in_range(timerange)
        else:
    ...
    def get_repository_health(self, timerange: TimeRange | None = None) -> RepositoryHealth:
        """Get overall repository health assessment.

        Synthesizes all signals (test, flaky, coverage, etc.) to produce
        a comprehensive health status for repository state.
        ...
        Example:
            health = query.get_repository_health(TimeRange.last_days(7))
            if health.status == "CRITICAL":
                autonomy.escalate(f"Repository health critical: {health.flaky_test_percent:.1f}% flaky")
        """
        if timerange:
            snapshots = self._load_snapshots_in_range(timerange)
        else:
    ...
    def filter_by_category(
        self, category: str, timerange: TimeRange | None = None
    ) -> list[FlakyTest]:
        """Get flaky tests filtered by flakiness category.

        Filters FlakyTest results to show only tests matching a specific
        flakiness category (INTERMITTENT, ENVIRONMENT, INFRASTRUCTURE, UNKNOWN).
        ...
        Example:
            env_issues = query.filter_by_category("ENVIRONMENT", TimeRange.last_days(7))
            for test in env_issues:
                autonomy.investigate_environment_sensitivity(test.name)
        """
