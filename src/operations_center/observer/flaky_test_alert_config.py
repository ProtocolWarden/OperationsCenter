...
class AlertChannelConfig:
    ...
    alert_type: str
    info_channels: list[str] = dataclass_field(default_factory=list)
    warning_channels: list[str] = dataclass_field(default_factory=list)
    critical_channels: list[str] = dataclass_field(default_factory=list)
    emergency_channels: list[str] = dataclass_field(default_factory=list)

    def get_channels_for_severity(self, severity: str) -> list[str]:
        """Get channels to notify for a given severity level."""
        severity_map = {
            "INFO": self.info_channels,
            "WARNING": self.warning_channels,
            "CRITICAL": self.critical_channels,
            "EMERGENCY": self.emergency_channels,
        }
        return severity_map.get(severity.upper(), self.warning_channels)
...
class FlakyTestAlertConfig:
    ...
    def __init__(self) -> None:
        ...
        self.channel_routes: dict[str, AlertChannelConfig] = {
            "NEW_FLAKY_TEST": AlertChannelConfig(
                ...
                emergency_channels=["operator_log", "slack", "email"],
            ),
            ...
            "CRITICAL_FLAKINESS": AlertChannelConfig(
                ...
                info_channels=["operator_log"],
                warning_channels=["operator_log", "slack"],
                critical_channels=["operator_log", "slack", "email", "github"],
                emergency_channels=["operator_log", "slack", "email", "github", "pagerduty"],
            ),
        ...
        self.thresholds: dict[str, AlertThreshold] = {
            ...
            "regression_spike": AlertThreshold(
                alert_type="regression_spike",
                info_threshold=0.2,  # 20% increase
    ...
    def get_channels_for_alert(
        ...
        if not route:
            ...
            if severity.upper() in ("CRITICAL", "EMERGENCY"):
                return ["operator_log", "slack"]
            return ["operator_log"]

        return route.get_channels_for_severity(severity)

    def get_threshold(self, metric_name: str, severity: str = "WARNING") -> float:
        ...
        severity_map = {
            "INFO": threshold.info_threshold,
            "WARNING": threshold.warning_threshold,
            "CRITICAL": threshold.critical_threshold,
            "EMERGENCY": threshold.emergency_threshold,
        }
        return severity_map.get(severity.upper(), threshold.warning_threshold)
    ...
    def should_alert_on_flaky_count(self, count: int) -> tuple[bool, str]:
        ...
        """
        if count >= self.get_threshold("flaky_test_count", "EMERGENCY"):
            return True, "EMERGENCY"
        if count >= self.get_threshold("flaky_test_count", "CRITICAL"):
            return True, "CRITICAL"
        if count >= self.get_threshold("flaky_test_count", "WARNING"):
            return True, "WARNING"
        return False, ""
    ...
    def should_alert_on_failure_rate(self, rate: float) -> tuple[bool, str]:
        ...
        Returns:
            Tuple of (should_alert, severity)
        """
        if rate >= self.get_threshold("failure_rate", "EMERGENCY"):
            return True, "EMERGENCY"
    ...
    def should_alert_on_regression(self, increase_percent: float) -> tuple[bool, str]:
        ...
        Returns:
            Tuple of (should_alert, severity)
        """
        if increase_percent >= self.get_threshold("regression_spike", "EMERGENCY"):
            return True, "EMERGENCY"
        if increase_percent >= self.get_threshold("regression_spike", "CRITICAL"):
            return True, "CRITICAL"
        if increase_percent >= self.get_threshold("regression_spike", "WARNING"):
            return True, "WARNING"
        return False, ""
