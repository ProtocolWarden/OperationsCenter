...
# ── Escalation prevention helpers (CI thrash mitigation) ─────────────────────


def _compute_backoff_interval(level: int) -> int:
    """Compute exponential backoff interval in seconds.

    Levels: 0→5s, 1→10s, 2→20s, 3+→20s (capped).
    """
...
def _classify_missing_checks(
    ...
    """Classify missing checks into: never_registered, late_registering, stuck.

    Returns: (never_registered, late_registering, stuck)
    """
    if required_checks_configured is None:
    ...
    for check in missing_required:
        in_history = check in history
        in_configured = check in required_checks_configured

        if in_history:
            stuck.append(check)
        elif in_configured:
            late_registering.append(check)
        else:
            never_registered.append(check)

    return never_registered, late_registering, stuck
...
def _should_escalate_ci_wait(
    ...
    ci_flakiness_threshold_pct: float = 30,
    required_checks_configured: list[str] | None = None,
) -> tuple[bool, str | None]:
    ...
    history = state.get("ci_check_history", {})
    ci_wait_cycles = state.get("ci_wait_cycles", 0)

    never_reg, late_reg, stuck = _classify_missing_checks(
        state, missing_required, required_checks_configured
    )

    # Never-registered checks: escalate immediately (config error)
    if never_reg:
        return (False, None)  # Don't escalate yet; just track

    # Late-registering checks: use 60-cycle threshold
    if late_reg:
        if ci_wait_cycles >= ci_wait_cycles_first_registration:
            return (True, "ci_never_settled_late_registration")
        return (False, None)

    # Stuck checks: analyze failure pattern
    for check in stuck:
        ch = history.get(check, {})
        times_passed = ch.get("times_passed", 0)
        times_failed = ch.get("times_failed", 0)
        total_attempts = times_passed + times_failed

        if total_attempts > 0:
            failure_rate = (times_failed / total_attempts) * 100
        else:
            failure_rate = 0

        # Dense failure pattern (>= threshold)
        if failure_rate >= ci_flakiness_threshold_pct:
            if ci_wait_cycles >= ci_wait_cycles_already_seen:
                return (True, "ci_persistently_red_dense_failure")

    # All checks below escalation thresholds
    if ci_wait_cycles >= ci_wait_cycles_already_seen and missing_required:
...
def _normalize_concerns_signature(summary: str) -> str:
    """Create normalized signature for concern deduplication.

    Removes line numbers and other variable parts, keeping core concern type.
    """
    # Replace numbers with placeholder
    normalized = re.sub(r"\d+", "N", summary)
    # Normalize line references
    normalized = re.sub(r"line\s+\d+", "line N", normalized)
    return normalized.lower().strip()


def _track_concern_raised(state: dict, summary: str, head_sha: str) -> None:
    ...
    state["last_concerns_head_sha"] = head_sha
