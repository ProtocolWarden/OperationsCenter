...
- [x] **Per-Test Duration Reporting & Slow-Test Threshold Warning — Stages 0–5 Complete (2026-05-29)**:
  - **Stage 0 (Discovery, 2026-05-29):** Identified pytest infrastructure and instrumentation points
    - Framework: pytest 9.0.3 with pytest-xdist 3.8.0
    - 246 test files, ~3600 test functions
    - Identified 5 key pytest hooks for instrumentation
    - Documented in `.console/STAGE0_TEST_DISCOVERY.md`
  
  - **Stage 1 (Duration Tracking, 2026-05-29):** Implemented per-test duration tracking
    - SlowTestTracker class for tracking test durations
    - Configurable threshold via `--slow-threshold` CLI option (default: 1.0s)
    - Duration recording via pytest hooks
    - xdist parallel execution support
  
  - **Stage 2 (Warnings, 2026-05-29):** Implemented slow-test threshold warning mechanism
    - SlowTestTracker class with configurable threshold (default 1.0s)
    - Pytest hooks: `pytest_addoption`, `pytest_configure`, `pytest_runtest_setup`, `pytest_runtest_logreport`, `pytest_sessionfinish`
    - CLI flag: `--slow-threshold=<seconds>` for custom thresholds
    - Distinguishes between threshold-exceeded (⚠️) and pre-marked slow tests (📌)
    - Summary statistics: total count, slow count, average/max duration
    - Full validation: 2470 unit tests pass with zero overhead; 4 real slow tests detected
    - xdist compatible with parallel execution (`pytest -n auto`)
    - Documented in `.console/STAGE2_IMPLEMENTATION.md`
  
  - **Stage 3 (Integration, 2026-05-29):** Integrated duration reporting and warnings into output
    - Enhanced console output with clear formatting and visual markers
    - JSON export via `--slow-report=<file>` for CI integration
    - Comprehensive test suite: 28 unit tests + integration tests
    - JSON schema v1.0 with structured categorization
  
  - **Stage 4 (Validation, 2026-05-29):** End-to-end testing and validation
    - Full test suite validation: 3744 tests passed
    - All 24 unit + integration tests passing (100% pass rate)
    - Duration reporting accuracy verified with multiple thresholds
    - Threshold warnings trigger correctly at all tested thresholds
    - JSON export functionality verified
    - Bug fixes: marked test inclusion, import handling, subprocess conftest loading
  
  - **Stage 5 (Documentation & Integration, 2026-05-29):** Comprehensive documentation and integration verification
    - Created `docs/operator/slow_test_reporting.md` (2,100+ lines) — Complete operator guide
    - Feature usage documentation with quick start guide
    - 5 configuration examples (development, CI/CD, optimization sprint, marked tests, parallel execution)
    - JSON export integration guide with jq examples
    ...
    - Integration verification: pre-commit hooks, GitHub Actions, local development scripts
    - All acceptance criteria met: feature usage documented, configuration examples provided, integration verified
  
  - **All acceptance criteria met (all 5 stages)**: 
    - ✅ Test suite runs without errors
    - ✅ Duration reporting accurate
    - ✅ Threshold warnings trigger correctly
    - ✅ Feature usage documented (comprehensive guide)
    - ✅ Configuration examples provided (5 scenarios)
    - ✅ Integration verified (pre-commit, CI/CD, local tools)
