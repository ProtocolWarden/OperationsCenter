...
def test_score_proposal_utility_components():
    ...
    high = _score_proposal_utility(
        ...
        family_recency_hours=0,
        repo_priority=10,
    )
    low = _score_proposal_utility(
        family_acceptance_rate=0.0,
        family_recency_hours=200,
        repo_priority=0,
    )
    assert high > low
    # All-zeros = 0
    assert (
        _score_proposal_utility(family_acceptance_rate=0, family_recency_hours=0, repo_priority=0)
        == 0.3
    )  # recency=full when hours=0
...
def test_score_proposal_utility_clamps_inputs():
    ...
    s = _score_proposal_utility(
        family_acceptance_rate=2.0,
        family_recency_hours=-50,
        repo_priority=999,
    )
    assert 0.0 <= s <= 1.0
