...
def _write_decision(
    root: Path, run_id: str, *, candidates: list, suppressed: list, dry_run: bool = False
) -> None:
    d = root / run_id
...
def test_excludes_dry_run_artifacts(tmp_path: Path) -> None:
    ...
    _write_decision(
        dec,
        "dec_dry",
        candidates=[{"family": "observation_coverage", "status": "emit"}],
        suppressed=[],
        dry_run=True,
    )
    _write_decision(
        dec, "dec_real", candidates=[{"family": "test_visibility", "status": "emit"}], suppressed=[]
    )
...
def test_suppression_rate_zero_when_no_suppressions(tmp_path: Path) -> None:
    dec = tmp_path / "decision"
    _write_decision(
        dec, "dec_1", candidates=[{"family": "dependency_drift", "status": "emit"}], suppressed=[]
    )
...
def test_create_rate_zero_when_none_created(tmp_path: Path) -> None:
    dec = tmp_path / "decision"
    _write_decision(
        dec, "dec_1", candidates=[{"family": "test_visibility", "status": "emit"}], suppressed=[]
    )
...
def test_window_limits_artifact_count(tmp_path: Path) -> None:
    ...
    for i in range(10):
        _write_decision(
            dec,
            f"dec_{i:02d}",
            candidates=[{"family": "test_visibility", "status": "emit"}],
            suppressed=[],
        )

    metrics, runs, _, _ = aggregate_family_metrics(
        decision_root=dec, proposer_root=tmp_path / "p", window=3
    )
    assert runs == 3
