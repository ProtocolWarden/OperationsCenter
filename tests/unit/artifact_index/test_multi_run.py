...
class TestDiscoverManifestFiles:
    ...
    def test_finds_nested_manifests(self, tmp_path: Path, completed_manifest_payload) -> None:
        ...
        b1 = _write_bucket(
            tmp_path, audit_type="audit_type_1", run_id="r1", payload_factory=_make_manifest_payload
        )
        b2 = _write_bucket(
            tmp_path, audit_type="enrichment", run_id="r2", payload_factory=_make_manifest_payload
        )
...
class TestFailureHandling:
    def test_corrupt_manifest_recorded_with_load_error(self, tmp_path: Path) -> None:
        ...
        _write_bucket(
            tmp_path, audit_type="audit_type_1", run_id="ok", payload_factory=_make_manifest_payload
        )
    ...
    def test_partial_manifest_indexes_normally(self, tmp_path: Path) -> None:
        ...
        _write_bucket(
            tmp_path, audit_type="audit_type_1", run_id="r_partial", payload_factory=partial_factory
        )
        idx = build_multi_run_index(tmp_path)
...
class TestResolve:
    def test_resolve_returns_path_for_existing_artifact(self, tmp_path: Path) -> None:
        from tests.unit.artifact_index.conftest import (  # type: ignore
            _BASE_ENTRY,
            _make_manifest_payload,
        )

        _bucket = _write_bucket(
            tmp_path, audit_type="audit_type_1", run_id="r1", payload_factory=_make_manifest_payload
        )
...
class TestQueryFederation:
    def test_query_federates_across_runs(self, tmp_path: Path) -> None:
        from tests.unit.artifact_index.conftest import (
            _BASE_ENTRY,
            _SINGLETON_ENTRY,
            _make_manifest_payload,
        )  # type: ignore
        ...
        _write_bucket(
            tmp_path, audit_type="audit_type_1", run_id="r1", payload_factory=two_artifact_factory
        )
        _write_bucket(
            tmp_path, audit_type="audit_type_1", run_id="r2", payload_factory=two_artifact_factory
        )
