...
def test_get_pr_diff_follows_redirect() -> None:
    ...
    with patch("operations_center.adapters.github_pr.httpx.get", return_value=mock_resp) as mock_get:
...
def test_get_pr_diff_returns_empty_on_error() -> None:
    ...
    client = _make_client()
    with patch("operations_center.adapters.github_pr.httpx.get", side_effect=Exception("network error")):
